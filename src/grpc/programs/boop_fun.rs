use crate::build_tx::boop_fun::get_instruction_accounts;
use crate::build_tx::boop_fun::BoopFunAccounts;
use crate::build_tx::boop_fun::build_boop_fun_swap_instruction;
...
use crate::build_tx::boop_fun::SwapDirection;

pub fn boop_fun_build_buy_tx(
...
) -> (Instruction, Pubkey, u64, BoopFunAccounts) {
    ...
    let boop_fun_accounts = get_instruction_accounts(&instruction_accounts);
    ...
    let buy_instruction = build_boop_fun_swap_instruction(
        &boop_fun_accounts,
        SwapDirection::Buy,
        amount,  // SOL amount being spent
        (expected_token_amount as f64 * slippage_factor) as u64,  // Minimum token amount expected to receive
    );

    println!("[BOOP FUN] DEBUG - SOL amount: {}, Expected token amount: {}", amount, expected_token_amount);
